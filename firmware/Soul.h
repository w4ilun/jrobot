...
private:
    ...
    inline static const int SLOT_GETUP_FACE_UP()
    {
        return 88;
    }

    //! @brief Slot of get up (face down) motion
    inline static const int SLOT_GETUP_FACE_DOWN()
    {
        return 89;
    }

    //! @brief Beginning slot of random motion patterns
    inline static const int MOTIONS_SLOT_BEGIN()
    {
        return 83;
    }

    //! @brief Ending slot of random motion patterns
    inline static const int MOTIONS_SLOT_END()
    {
        return 88;
    }

    //! @brief The interval which makes stated periods
    inline static const int BASE_INTERVAL_MSEC()
    {
        return 15000;
    }

    //! @brief The interval which gives flicker to base-interval
    inline static const int RANDOM_INTERVAL_MSEC()
    {
        return 10000;
    }

    //! @brief Wait time for getting up automatically
    inline static const int GETUP_WAIT_MSEC()
    {
        return 2000;
    }

    //! @brief Sampling interval
    inline static const int SAMPLING_INTERVAL_MSEC()
    {
        return 100;
    }

    //! @brief A threshold to decide gravity axis
    inline static const int GRAVITY_AXIS_THRESHOLD()
    {
        return 13000;
    }
