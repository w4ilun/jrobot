...
private:
    ...
    inline static const int INIT_FLAG_ADDRESS()
    {
        return 0;
    }

    //! @brief Initialized flag's value
    inline static const unsigned char INIT_FLAG_VALUE()
    {
        return 2;
    }

    //! @brief Head-address of joint settings on internal EEPROM
    inline static const int SETTINGS_HEAD_ADDRESS()
    {
        return 1;
    }
...
public:
    ...
    */
    ...
    public:
        ...
        inline static const int SELECTABLE_LINES()
        {
            return 8;
        }
    };
#if USE_DIGTAL_SERVO
    inline static const int PWM_FREQ()
    {
        return 320;
    }

    //! @brief PWM width that to make min angle
    inline static const int PWM_MIN()
    {
        return 1100;
    }

    //! @brief PWM width that to make max angle
    inline static const int PWM_MAX()
    {
        return 2700;
    }

    //! @brief PWM width that to make neutral angle
    inline static const int PWM_NEUTRAL()
    {
        return 1900;
    }
#else
